...
export default function MyMoviesPage() {
    ...
    const movies = useMemo(() => {
        ...
        const items = watchlist.filter(item => item.media_type === 'movie');
    ...
    return (
        <div className="min-h-screen bg-[#0a0a0a] text-white font-sans pb-20">
            <header className="fixed top-0 left-0 right-0 h-16 bg-[#0a0a0a]/90 backdrop-blur z-40 border-b border-gray-800 flex items-center px-6">
                {/* Reusing Main Layout Header logic is not possible without refactor, 
                 so we rely on the Root Layout's MainLayout wrapper which provides the header.
                 BUT MainLayout renders children inside <main>, so we don't need a header here.
                 The user requested "Routing must mirror...". The existing pages are wrapped in MainLayout.
                 So here we just render content. 
                 Wait, MainLayout HAS a header.
                 So I should NOT render a header here.
                 */}
            </header>

            {/* 
                CORRECTION: MainLayout wraps children.
                So I just need to render the content div.
                The previous watchlist page had <main> inside.
            */}

            <main className="max-w-7xl mx-auto px-6 py-12">
                <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4 border-b border-gray-800 pb-6">
                    <div>
                        {/* Breadcrumb-ish link back to hub? Or just title? User asked for "My Movies" page. */}
                        <div className="flex items-center gap-2 mb-2">
                            <Link href="/watchlist" className="text-gray-500 hover:text-white transition-colors">My List</Link>
                            <span className="text-gray-600">/</span>
                            <h1 className="text-4xl font-bold">My Movies</h1>
                        </div>
                        <p className="text-gray-400">{movies.length} titles saved</p>
                    </div>
                ...
                {movies.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-20 border border-dashed border-gray-800 rounded-2xl bg-[#111]">
                        ...
                        <h3 className="text-xl font-bold text-gray-300 mb-2">No movies yet</h3>
                        <p className="text-gray-500 mb-6">Start adding movies to track them here.</p>
                        <Link href="/movies" className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-full font-bold transition-colors shadow-lg shadow-blue-900/20">
                            Browse Movies
                        </Link>
                    </div>
                ) : (
